...
class Stacker():
    ...
    def _single_plotter(ax, stacked_seds_tmp, kw, line_label=None, 
        ...
        for k in range(n_lines):
            ...
            if counts:
                ...
                ax.fill_between(x, y+y_err, y-y_err, color=color, alpha=0.3, label='_nolegend_')
        ...
        if spectral_lines_dict:
            ...
            for key, wls in spectral_lines_dict.items():
                ...
                same_line = False
                for wl in wls:
                    line = ax.axvline(wl, color=color, linewidth=1, linestyle='-.')
                    if not same_line:
                        lines.append(line)
                        same_line = True
    ...
    def load_catalog(self, catalog, max_nan_bands=0, z_label='zb', 
        ...
        if not bands_data:
            ...
            raise ValueError("'bands_data' must be a string (directory of .json file) or dict")               
            
            
        self.nb_err_labels = [label + bands_error_suffix for label in self.nb_labels]
        
        # Removing objects with more nan bands than max_nan_bands
        select = np.sum(np.isnan(df[self.nb_labels].values), axis=1) <= max_nan_bands
    ...
    def load_stack(self, stack_folder):
        ...
        self.stacked_seds = xr.open_dataarray(f'{self.stack_folder}stacked_seds.nc')
    ...
    def column_histogram(self,label,  bins, save=False):
        ...
        try:
            ...
            raise Exception("No catalog loaded. Please run load_catalog() first.")
        ...
        if save:
            ...
            plt.savefig(f'{folder}{label}_hist.pdf', bbox_inches='tight', transparent=True)
            plt.savefig(f'{folder}{label}_hist.png', bbox_inches='tight', transparent=True)
    ...
    def to_rest_frame(self, flux_conversion='normalized', use_band_responses=False,
        ...
        if flux_conversion not in ['normalized', 'luminosity', 'nothing']:
            raise Exception("flux_conversion not understood. Please input 'normalized', 'luminosity' or 'nothing'")
        ...
        rf_seds = np.zeros([len(self.df), wl_grid.shape[0]])
        if compute_error:
            rf_seds_err = np.zeros([len(self.df), wl_grid.shape[0]])
        seds = self.df[self.nb_labels].values
        seds_err = self.df[self.nb_err_labels].values
        ...
        if compute_error:
            self.rf_seds_err = np.ma.array(rf_seds_err, mask=np.isnan(rf_seds))
    ...
    def stack(self, bin_dict={}, weight=None, error_type=None, min_n_obj=0):
        ...
        try:
           self.rf_seds 
            ...
            raise Exception('SEDs not shifted to rest frame. Please run to_rest_frame() first')
        ...
        for x in it:
            ...
            select = np.ones(len(self.df)).astype(bool)
            for i, item in enumerate(it.multi_index):
                bin_edg = bin_edgs[i][item]
                if discrete_bins[bin_labels[i]]:
                    select *= self.df[bin_labels[i]] == bin_edg
                else:
                    select *= (self.df[bin_labels[i]] >= bin_edg[0]) & (self.df[bin_labels[i]] < bin_edg[1])
            ...
            if error_type:
                if error_type.lower() == 'flux_error':                        
                    ...
                    variance = np.ma.average((rf_seds_tmp - stack_sed)**2, weights=weights_tmp, axis=0)
    ...
    def plot(self, line_label=None, column_label=None, row_label=None, 
        ...
        if line_label:
            ...
            legend_labels = [self._range_label(line_label, i) for i in range(n_lines)]
        else:
            n_lines = 1
            legend_labels = None
        ...
        it = np.nditer(stacked_seds_cropped.isel(**kw, data=0, rf_wl=0), flags=['multi_index'])
        for _ in it:
            inds = it.multi_index
            ...
            filename = [f'{label}{inds[i]}' for i, label in enumerate(fig_labels)]
            if subplot_label:
            ...
            fig, ax = plt.subplots(nrows=n_rows, ncols=n_cols, dpi=160, sharex=True, sharey=sharey,
                                  figsize=[6.4 * n_cols, 6.4 * n_rows / aspect_ratio],
            ...
            if fig_title:
                suptitle = [self._range_label(label, inds[i]) for i, label in enumerate(fig_labels)]
                suptitle = ', '.join(suptitle)
                if suptitle != '':
                    plt.suptitle(suptitle, wrap=True, fontsize=fontsize, y=0.95)
